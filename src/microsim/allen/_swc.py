...
class SWC:
    ...
    def from_string(cls, content: str | bytes) -> SWC:
        ...
        for num, line in enumerate(content.splitlines()):
            ...
            try:
                ...
                comp = Compartment(int(a), int(b), float(c), float(d),
    ...
    def transformed_coords(self, coords, dim=None) -> np.ndarray:
        """Returns: Transformed coordinates to the new extent"""
        if dim is None:
            return coords
        else:
            ...
            print("Current extent:", current_extent)
            scale = dim / current_extent
            transformed_coords = np.ceil((coords - min_coords) * scale)
            print("Transformed extent:", np.ptp(transformed_coords, axis=0))
            transformed_coords = transformed_coords.astype(int)
            return transformed_coords, scale, min_coords
    ...
    def soma_location(self) -> tuple[float, float, float]:
        ...
        self._node_types[SWCType.SOMA]
        return self._node_types[SWCType.SOMA]

    def offset_soma(self, offset: tuple[float, float,
                                float]) -> tuple[float, float, float]:
        """Offset the soma location by a given amount."""
    ...
    def rotate_soma(self, rotation: tuple[float, float, float]) -> np.ndarray:
        """Rotate the coordinates of the soma of the neuron reconstruction."""
    ...
    def scale_soma(self, scale: float) -> np.ndarray:
    ...
    def bounding_box(
        ...
        return (min_coords[0], max_coords[0]), (min_coords[1],
    ...
    def create_truth_space(
        ...
        rotation: tuple[float, float, float] = (8, 185, 90),
        *,
        include_types: Iterable[int] = (
            SWCType.BASAL_DENDRITE,
            SWCType.APICAL_DENDRITE,
        ...
        coordinates, scale, min_coords = self.transformed_coords(self.coords,
                                                                 dim=dimension)
        origin = np.min(coordinates, axis=0)  #min of all the x,y,z coordinates
        grid = self._empty_grid(voxel_size, coordinates=coordinates)
        grid_center = np.array(grid.shape) / 2

        #create rotation-translation matrix
        rx = np.array(
            [[1, 0, 0, grid_center[2]],
             [0, np.cos(rotation[0]), -np.sin(rotation[0]), grid_center[1]],
             [0, np.sin(rotation[0]),
              np.cos(rotation[0]), grid_center[0]], [0, 0, 0, 1]])
        ry = np.array(
            [[np.cos(rotation[1]), 0,
              np.sin(rotation[1]), grid_center[2]], [0, 1, 0, grid_center[1]],
             [-np.sin(rotation[1]), 0,
              np.cos(rotation[1]), grid_center[0]], [0, 0, 0, 1]])
        rz = np.array(
            [[np.cos(rotation[2]), -np.sin(rotation[2]), 0, grid_center[2]],
             [np.sin(rotation[2]),
              np.cos(rotation[2]), 0, grid_center[1]],
             [0, 0, 1, grid_center[0]], [0, 0, 0, 1]])
        ...
        soma_scale: float = 1
        for comp in self._node_types[SWCType.SOMA]:
            z, y, x = comp.shifted_coord(origin, voxel_size, scale, min_coords)
            soma_center = np.array([z, y, x])
            offset = grid_center - soma_center
            soma_center = soma_center  #+ offset

            # soma_center = rotation_matrix.dot(soma_center)
            #append a 1 to the end of the vector
            # soma_center = np.append(soma_center, 1)
            draw_sphere(grid, soma_center[2], soma_center[1], soma_center[0],
                        np.prod(scale))

        for par, child in self.iter_pairs(*include_types):
            pz, py, px = par.shifted_coord(origin, voxel_size, scale,
                                           min_coords)  #+ offset

            #rotate pz
            # pz, py, px = rotation_matrix.dot(np.array([np.append(pz,1), np.append(py,1), np.append(px, 1)]))

            cz, cy, cx = child.shifted_coord(origin, voxel_size, scale,
                                             min_coords)  #+ offset
            #rotate cz
            # cz, cy, cx = rotation_matrix.dot(np.array([np.append(cz, 1), np.append(cy, 1), np.append(cx, 1)]))
